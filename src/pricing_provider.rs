...
pub async fn fetch_real_lambda_price(region: &str, architecture: &str) -> Result<f64> {
    ...
    for json_str in price_list {
        if let Ok(v) = serde_json::from_str::<Value>(json_str) {
            ...
            if let Some(attributes) = v.get("product").and_then(|p| p.get("attributes")) {
                let usage_type = attributes.get("usagetype").and_then(|u| u.as_str()).unwrap_or("");
                let proc_arch = attributes.get("processorArchitecture").and_then(|a| a.as_str()).unwrap_or("");
                ...
                if is_arm64 {
                    ...
                    if !usage_type.contains("ARM") && (proc_arch == "x86_64" || proc_arch == "AMD64" || proc_arch.is_empty()) {
                        matches_arch = true;
                    }
                }
            }

            if !matches_arch { continue; }
            ...
            if let Some(terms) = v.get("terms").and_then(|t| t.get("OnDemand")) {
                if let Some(on_demand_obj) = terms.as_object() {
                    if let Some(first_offer) = on_demand_obj.values().next() {
                        if let Some(dimensions) = first_offer.get("priceDimensions").and_then(|d| d.as_object()) {
                            if let Some(first_dimension) = dimensions.values().next() {
                                if let Some(price_str) = first_dimension.get("pricePerUnit").and_then(|p| p.get("USD")).and_then(|u| u.as_str()) {
    ...
    found_price.ok_or_else(|| anyhow!("Could not find pricing data for architecture: {}", architecture))
}
...
pub async fn calculate_costs(size_mb: f64, executions: u64, memory_mb: u32, region: &str, architecture: &str) -> PricingEstimate {
    ...
    let (lambda_gb_second, dynamic_pricing_used) = match fetch_real_lambda_price(region, architecture).await {
        Ok(price) => (price, true),
        Err(_) => {
