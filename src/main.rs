...
async fn main() -> Result<()> {
    ...
    match args.command {
        BillSubcommands::Lambda(lambda_args) => {
            ...
            println!("Region: {}, Memory: {} MB, Executions: {}, Architecture: {}", lambda_args.region, lambda_args.memory, lambda_args.executions, lambda_args.architecture);
            ...
            let costs = pricing_provider::calculate_costs(analysis.size_mb, lambda_args.executions, lambda_args.memory, &lambda_args.region, &lambda_args.architecture).await;

            if analysis.size_mb > 30.0 {
                let has_heavy_deps = metadata.packages.iter().any(|p| p.name.contains("aws-sdk"));
                if has_heavy_deps {
                    println!("\nWarning: Your binary is unusually large ({:.2} MB).", analysis.size_mb);
            ...
            table.add_row(row!["Has Debug Symbols", if analysis.has_debug_symbols { "Yes" } else { "No" }]);
            table.add_row(row![
            ...
            table.add_row(row![
                ...
                if costs.dynamic_pricing_used { "Yes" } else { "No (Fallback)" }
            ]);
